...
public abstract class FrameworkServlet extends HttpServletBean implements ApplicationContextAware {
	...
	protected final void initServletBean() throws ServletException {
		...
		try {
			//todo 初始化web环境中的spring容器WebApplicationContext
	 ...
	 * @see #setContextClass
	 * @see #setContextConfigLocation
	 * todo 初始化WebApplicationContext
	 */
	protected WebApplicationContext initWebApplicationContext() {
		WebApplicationContext rootContext =
				WebApplicationContextUtils.getWebApplicationContext(getServletContext());
		WebApplicationContext wac = null;
		//todo 寻找或创建对应的WebApplicationContext实例
		if (this.webApplicationContext != null) {
			// A context instance was injected at construction time -> use it
			//todo context 实例在构造函数中被注入
		...
		if (wac == null) {
			...
			//todo 根据contextAttribute属性加载WebApplicationContext
			wac = findWebApplicationContext();
		}
		if (wac == null) {
			//todo 如果通过以上两种方式并没有找到任何突破，那就没办法了，只能在这里重新创建新的实例了
	 ...
	 * todo 根据contextAttribute属性加载WebApplicationContext
	 * todo 通过在web.xml文件中配置的servlet参数contextAttribute来查找ServletContext中对应的属性，
	 * todo 默认为WebApplicationContext.class.getName() +".ROOT",也就是在ContextLoaderListener加载时会创建
	 * todo WebApplicationContext实例,并将实例以WebApplicationContext.class.getName() + ".ROOT”为key放入
	 * todo ServletContext中，当然读者可以重写初始化逻辑使用自己创建的 WebApplicationContext,并在 servlet
	 * todo 的配置中通过初始化参数 contextAttribute 指定 key
	 */
	@Nullable
	 ...
	 * @see org.springframework.web.context.support.XmlWebApplicationContext
	 * todo 创建WebApplicationContext
	 */
	protected WebApplicationContext createWebApplicationContext(@Nullable ApplicationContext parent) {
		//todo 获取 servlet 的初始化参数contextClass 如果没有配置默认为 XmlWebApplicationContext .class
		...
		//todo 创建web应用上下文 通过反射方式实例化contextclass
		ConfigurableWebApplicationContext wac =
				(ConfigurableWebApplicationContext) BeanUtils.instantiateClass(contextClass);

		wac.setEnvironment(getEnvironment());
		//todo parent 为在ContextLoaderListener中创建的实例
		//todo 在 ContextLoaderListener加栽的时候初始化的WebApplicationContext类型实例
		wac.setParent(parent);
		
		//todo 获取上下文配置文件路径
		String configLocation = getContextConfigLocation();
		if (configLocation != null) {
			//todo 获取contextConfigLocation属性.配置在servlet初始化参数中
			wac.setConfigLocation(configLocation);
		}
		//todo 调用容器初始化方法，完成web环境中的spring容器初始化 初始化Spring环境包括加载配置文件等
	...
	protected void configureAndRefreshWebApplicationContext(ConfigurableWebApplicationContext wac) {
		...
		//todo 调用容器初始化方法 加载配置文件及整合parent到wac
		//todo onRefresh是FrameworkServlet类中提供的模板方法，在其子类DispatcherServlet中进行了重写，
		//todo 主要用于刷新Spring在Web功能实现中所必须使用的全局变量
		wac.refresh();
	}
	 ...
	 * @see #refresh()
	 * todo 在其子类DispatcherServlet中进行了重写
	 */
	 ...
	 * @see #doService
	 * @see #doHead
	 * todo 对于不同的方法，Spring并没有做特殊处理，而是统一将程序再一次地引导至processRequest(request, response)
	 */
	@Override
	...
	protected final void processRequest(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		//todo 记录当前时间.用于计算web请求的处理时间
		long startTime = System.currentTimeMillis();
		Throwable failureCause = null;
		//todo (1 )为了保证当前线程的LocaleContext以及RequestAttributes可以在当前请求后还能恢复，提取当前线程的两个属性
		...
		//todo (2 )根据当前request创建对应的LocaleContext和RequestAttributes,并绑定到当前线程
		initContextHolders(request, localeContext, requestAttributes);

		try {
			//todo (3)委托给doService方法进一步处理
