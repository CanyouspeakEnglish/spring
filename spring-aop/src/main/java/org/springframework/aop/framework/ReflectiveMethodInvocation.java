...
public class ReflectiveMethodInvocation implements ProxyMethodInvocation, Cloneable {
	 ...
	 * @throws Throwable
	 *
	 * todo ReflectiveMethodlnvocation 中的主要职责是维护了链接调用的汁数器，记录者当前调用链接的位置.
	 * todo 以便链可以有序地进行下去，那么在这个方法中并没有我们之前设想的维护各种增强的顺序，而是将此工作委托给了各个增强器
	 * todo ，使各个增强器在内部进行逻辑实现
	 */
	@Override
	@Nullable
	public Object proceed() throws Throwable {
		...
		if (interceptorOrInterceptionAdvice instanceof InterceptorAndDynamicMethodMatcher) {
			 ...
			 * 普通拦截器，直接调用拦截器,比如：
			 ...
			 •DelegatePerTargetObjectintreductionInterceptor,
			...
			//todo 将this作为参数传递以保证当前实例中凋用链的执行return((Methodinterceptor)
