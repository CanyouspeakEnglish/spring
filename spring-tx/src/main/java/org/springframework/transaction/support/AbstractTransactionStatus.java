...
public abstract class AbstractTransactionStatus implements TransactionStatus {
	 ...
	 * todo 当之前已经保存的事务信息中有保存点信息的时候，使用保存点信息进行回滚.常用于嵌入式事务，
	 * todo 对于嵌入式的事务的处理，内嵌的事务异常并不会引起外部事务的回滚.
	    todo 根据保存点回滚的实现方式其实是根据底层的数据库连接进行的
	 */
	public void rollbackToHeldSavepoint() throws TransactionException {
		...
		//todo 这里使用的是JDBC的方式进行数据库连接，那么getSavepointManager函数返回的是
		//todo JdbcTransactionObjectSupport.也就是说上面函数会调用 JdbcTransactionObjectSupport 中的
		//todo  rollbackToSavepoint 方法
