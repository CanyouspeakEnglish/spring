 ...
 * todo jdbc的执行流程
 * 1  在Java程序中加载驱动程序 Class.forName("com.mysql.jdbc.Driver")
 * 2 创建数据连接对象 Connection connection = DriverManager.geiConnection("连接数据库的URL”,”用户名”,”密码”)
 * 3 创建Statement对象 Statement statamentMySQL = connection.createStatement();
 * 4 调用Statement对象的相关方法执行相对应的SQL语句  statement.excuteUpdate() HesultSet resultSel =  statement.executeQuery
...
public class JdbcTemplate extends JdbcAccessor implements JdbcOperations {
	 ...
	 * todo 这里直接使用connection创建， 而带有参数的SQL使用的是PreparedSlatementCreator
	 * todo 类来创建的。一个是普通的Statement, 另一个是PreparedStatement,两者究竟是何区别呢
	 * todo PreparedStatement接口继承Statement,并与之在两方面有所不同
	 * todo PreparedStatement实例包含已编译的SQL语句,这就是使语句“准备好”.包含于 PreparedStatement对象中的SQL语句可具有一个或
	 * todo 多个IN参数。IN参数的值在SQL 语句创建时未被指定.相反的，该语句为每个IN参数保留一个问号(”?”)作为占位 符.每
	 * todo 个问号的值必须在该语句执行之前，通过适当的setXXX方法来提供
	 * todo 由于PreparedStatement对象已预编译过，所以其执行速度要快于Statement对象。因 此，
	 * todo 多次执行的SQL语句经常创建为PreparedStatement对象，以提高效率
	 * todo 作为Statement的子类，PreparedStatement继承了 Statement的所有功能：, 另外，它还添加 了一整套方法，
	 * todo 用于设置发送给数据库以取代IN参数占位符的值°同时，三种方法execute.
	todo executeQuery和executeUpdate已被更改以使之不再需要参数。这些方法的Statement形式
	todo (接 受SQL语句参数的形式)不应该用于PreparedStatement对象
	 */
	@Override
	@Nullable
	...
	/**
	 * todo 查询不带参数的 如select * from user
	...
	public <T> T execute(PreparedStatementCreator psc, PreparedStatementCallback<T> action)
		...
		try {
			...
			// Release Connection early, to avoid potential connection pool deadlock
			// in the case when the exception translator hasn't been initialized yet.
			//todo 释放敎据库连接避免当异常转换器没有被初始化的时候出现潜在的连接池死锁
	...
	public <T> T query(
		...
		//todo 回调类PreparedStatementCallback的实现使用的是ps.executeQuery
		return execute(psc, new PreparedStatementCallback<T>() {
			...
			public T doInPreparedStatement(PreparedStatement ps) throws SQLException {
				...
				try {
					if (pss != null) {
						//todo 设置PreparedStatement所需的全部参数
						pss.setValues(ps);
					}
					rs = ps.executeQuery();
					//todo rse.extractData(rsToUse)方法负责将结果进行封装并转换至POJO, rse当前代表的类为
					//todo RowMapperResultSetExtractor,而在构造 RowMapperResultSetExtractor
					//todo 的时候我们又将自定义 的rowMapper设置了进去
					return rse.extractData(rs);
				}
				finally {
					JdbcUtils.closeResultSet(rs);
	...
	public <T> T query(String sql, Object[] args, int[] argTypes, ResultSetExtractor<T> rse) throws DataAccessException {
		//todo 同样使用了newArgTypePreparedStatementSetter
	...
	protected int update(final PreparedStatementCreator psc, @Nullable final PreparedStatementSetter pss)
		...
		//todo execute方法是最基础的操作，而其他操作比如 update、query等方法则是传入不同的PreparedSlatementCallback参数来执行不同的逻辑
		return updateCount(execute(psc, ps -> {
			try {
				if (pss != null) {
					//todo 设置PreparedStatement所需要的全部参数
					pss.setValues(ps);
				}
	...
	public int update(String sql, @Nullable PreparedStatementSetter pss) throws DataAccessException {
		//todo 使用 SimplePreparedStatementCreator 对sql语句进行封装
		return update(new SimplePreparedStatementCreator(sql), pss);
	}

	@Override
	public int update(String sql, Object[] args, int[] argTypes) throws DataAccessException {
		//todo 使用 newArgTypePreparedStatementSetter 对参数和参数类型进行封账
	...
	protected void applyStatementSettings(Statement stmt) throws SQLException {
		...
		if (fetchSize != -1) {
			//todo setFetchSize最主要是为了减少网络交互次数设计的
			//todo 访问ResultSet时，如果它每次只从服务器上读取一行数据.则会产生大量的开销setFetchSize的意思是当调用rs.next时,
			//todo ResultSet 会一次性从服务器上取得多少行数据回来，这样在下次rs.next时，它可以直接从内存中获取数据而不需要网络交互，
			//todo 提高了效率。这个设置可能会被某些JDBC駆动忽略，而且设置过大 也会造成内存的上升
			stmt.setFetchSize(fetchSize);
		}
		int maxRows = getMaxRows();
		if (maxRows != -1) {
			//todo setMaxRows将此Statement对象生成的所有ResultSet对象可以包含的最大行数限制设置为给定数
	 ...
	 * @see org.springframework.jdbc.SQLWarningException
	 * todo 这里用到了一个类SQLWarning, SQLWarning提供关于数据库访问警告信息的异常。
	 * todo 这些警告直接链接到导致报告警告的方法所在的对象，警告可以从Connection. Statement和ResultSet
	 * todo 对象中获得。试图在已经关闭的连接上获取警告将导致抛出异常。类似地，试图在已经关闭的
	 * todo 语句上或已经关闭的结果集上获取警告也将导致抛出异常。注意，关闭语句时还会关闭它可能生成的结果集
	 * todo 很多人不是很理解什么情况下会产生警告而不是异常，在这里给读者提示个最常见的警告DataTruncation:
	 * todo DataTruncation直接继承SQLWarning.由于某种原因意外地截断数据值时会以DataTruncalion警告形式报告异常
	 * todo 对于警告的处理方式并不是直接抛出异常，出现警告很可能会出现数据错误，但是，并不 一定会影响程序执行，
	 * todo 所以用户可以自己设置处理警告的方式，如默认的是忽略警告，当出现警吿时只打印警告日志，而另一种方式只直接抛出异常
	 */
	protected void handleWarnings(Statement stmt) throws SQLException {
		//todo 当设置为忽略警告时只尝试打印日志
		if (isIgnoreWarnings()) {
			if (logger.isDebugEnabled()) {
				//todo 如果日志开启的情况下打印日志
