...
public abstract class DataSourceUtils {
	 ...
	 * todo 在数据库连接方面，Spring主要考虑的是关于事务方面的处理。基于事务处理的特殊性，
	 * todo Spring需要保证线程中的数据库操作都是使用同一个事务连接
	 */
	 ...
	 * (may be {@code null})
	 * @see #getConnection
	 * todo 数据库的连接释放并不是直接调用了Connection的API中的close方法。考虑到存在事务的情况，
	 * todo 如果当前线程存在事务，那么说明在当前线程中存在共用数据库连接,这种情况下直接使用
	 * todo ConnectionHolder中的released方法进行连接数减一，而不是真正的释放连接
	 */
	...
	public static void doReleaseConnection(@Nullable Connection con, @Nullable DataSource dataSource) throws SQLException {
		...
		if (dataSource != null) {
			//todo 当前线程存在事务的情况下说明存在共用数据库连接直接使用ConnectionHolder中的 released方法进行连接数减-而不是真正的释放连接
			ConnectionHolder conHolder = (ConnectionHolder) TransactionSynchronizationManager.getResource(dataSource);
