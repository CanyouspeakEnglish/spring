...
class LoadTimeWeaverBeanDefinitionParser extends AbstractSingleBeanDefinitionParser {
	...
	/**
	 * todo Spring解析到<context:load-time-weaver/> 后
	 * @param element the {@code Element} that is being parsed
	 * @return
	 * todo 会产生一个bean,而这个bean的id为loadTimeWeaver,class为org.Springframework.context.weaving.DefaultContextLoadTimeWeaver,
	 * todo 也就是完成了DefaultContextLoadTimeWeaver类的注册
	 */
	@Override
	...
	protected void doParse(Element element, ParserContext parserContext, BeanDefinitionBuilder builder) {
		...
		//todo 是否开启AspectJ 3个备选值，on、off和autodetect.默认为autodetect,也就是说，如果我们只是使用<context:load-time-weaver/>
		//todo Spring会帮助我们检测是否可以使用AspectJ功能,二检测的依据便是文件META-INF/aop.xml是否存在
		if (isAspectJWeavingEnabled(element.getAttribute(ASPECTJ_WEAVING_ATTRIBUTE), parserContext)) {
			//todo 将org.Springframework.context.weaving.AspectJWeavingEnabler 封装在 BeanDefinition 中注册
			if (!parserContext.getRegistry().containsBeanDefinition(ASPECTJ_WEAVING_ENABLER_BEAN_NAME)) {
				//todo 将类路径注册在新初始化的RootBeanDefinition中，在RootBeanDefinition的获取时会转换成对应的class
