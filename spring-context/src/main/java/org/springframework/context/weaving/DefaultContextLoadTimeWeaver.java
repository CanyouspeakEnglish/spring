 ...
 * DefaultContextLoadTimeWeaver类又同时实现了LoadTimeWeaver,BeanClassLoaderAware以及DisposableBean
 * 其中DisposableBean接口保证在bean销毁时会调用destroy方法进行bean 的清理，
 * 而 BeanClassLoaderAware 接口则保证在 bean 的初始化调用 AbstractAutowireCapableBeanFactory的invokeAwareMethods
 * 时调用 setBeanClassLoader 方法
 */
public class DefaultContextLoadTimeWeaver implements LoadTimeWeaver, BeanClassLoaderAware, DisposableBean {

	protected final Log logger = LogFactory.getLog(getClass());
	/**
	 * todo  初始化成 InstrumentationLoadTimeWeaver
	 */
	@Nullable
	...
	public void setBeanClassLoader(ClassLoader classLoader) {
		...
		if (serverSpecificLoadTimeWeaver != null) {
			...
			//todo 检查当前虚拟机中的Instrumentation实例是否可用
