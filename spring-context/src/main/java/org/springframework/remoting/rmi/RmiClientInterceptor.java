...
public class RmiClientInterceptor extends RemoteInvocationBasedAccessor
	...
	public void prepare() throws RemoteLookupFailureException {
		...
		//todo 如果配置了 lookupstubOnStartup属性便会在启动时寻找stub
	...
	protected Remote lookupStub() throws RemoteLookupFailureException {
		try {
			Remote stub = null;
			//todo 使用自定义的套接字工厂。如果使用这种方式，你需要在构造Registry实例时将自定义套接字工厂传人，
			//todo 并使用Registry中提供的lookup方法来获取对应的stub
			//todo 为了使用registryClientSocketFactory，代码量比使用RMI标准获取stub方法多出了很多，
			//todo 那么registryClientSocketFactory到底是做什么用的呢？
			if (this.registryClientSocketFactory != null) {
				...
				//todo 直接使用 RMI提供的标准方法：Naming.lookup(getServiceUrlO)
	 ...
	 * @throws RemoteLookupFailureException if RMI stub creation failed
	 * @see #lookupStub
	 * todo Spring中对于远程方法的调用其实是分 两种情况考虑的
	 * todo 获取的stub是RMIInvocationHandler类型的，从服务端获取的stub是RMIInvocationHandler,
	 * todo 就意味着服务端也同样使用了Spring去构建，那么自然会使用Spring中作的约定，进行客户端调用处理.
	 * todo Spring中的处理方式被委托給了dolnvoke方法
	 * todo 当获取的stub不是RMIInvocationHandler类型，那么服务端构建RMI服务可能是通过普通的方法或者借助于Spring外的第三方插件，
	 * todo 那么处理方式自然会按照RMI中普通的处理方式进行，而这种普通的处理方式无非是反射.因为在invocation中包含了
	 * todo 所需要调用的方法的各种信息，包括方法名称以及参数等，而调用的实体正是stub, 那么通过反射方法完全可以激活stub中的远程调用
 	 */
	 ...
	 * @see java.rmi.ConnectIOException
	 * @see java.rmi.NoSuchObjectException
	 * todo 当在客户端调用代理的接口中国某个方法时,就会首先执行RMIProxyFactoryBean的 invoke方法进行增强
	 */
	@Override
	public Object invoke(MethodInvocation invocation) throws Throwable {
		//todo 获取的服务器中对应的注册的remote对象 通过序列化传输
		Remote stub = getStub();
		try {
	 ...
	 * todo Spring中的客户端处理RMi的方式。其实,在分析服务端发布RMI的方式时,
	 * todo 我们已经了解到,Spring将RMI的导出Object封装成了RMIlnvocationHandler类型进行发布,
	 * todo 那么当客户端获取stub的时候是包含了远程连接信息代理类的RMIlnvocationHandler,
	 * todo 也就是说当调用RMIlnvocationHandler中的方法时会使用RMI中提供的代理进行远程连接,
	 * todo 而此时,Spring中要做的就是将代码引向RMIlnvocationHandler接口的invoke方法的调用
	 */
	@Nullable
	protected Object doInvoke(MethodInvocation methodInvocation, RmiInvocationHandler invocationHandler)
		...
		//todo 将methodlnvocation中的方法名及参数等信息重新封装Remotelnvocation,并通过远程代理方法直接调用
