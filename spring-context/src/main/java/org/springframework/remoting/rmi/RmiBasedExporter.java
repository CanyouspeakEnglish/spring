...
public abstract class RmiBasedExporter extends RemoteInvocationBasedExporter {
	 ...
	 * todo RMI会根据注册的服务名称，将请求引导至远程对象处理类中，这个处理类便是使用getObjectToExportOS进行创建
	 */
	protected Remote getObjectToExport() {
		// determine remote object
		//todo 如果配置的service属性对应的类实现了Remote接口且没有配置serviceInterface属性
		if (getService() instanceof Remote &&
				(getServiceInterface() == null || Remote.class.isAssignableFrom(getServiceInterface()))) {
			//todo 如果配置的service属性对应的类实现了 Remote接口且没有配置serviceInterface属性，那么直接使用service作为处理类
