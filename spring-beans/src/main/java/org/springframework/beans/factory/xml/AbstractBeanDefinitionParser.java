...
public abstract class AbstractBeanDefinitionParser implements BeanDefinitionParser {
	 ...
	 * @return
	 * todo BeanDefinitionParser parse方法
	 */
	@Override
	@Nullable
	public final BeanDefinition parse(Element element, ParserContext parserContext) {
		//todo 调用子类的parseInternal获取BeanDefinition对象
		//todo 真正去做解析的事情委托给了函数parseInternal  AbstractSingleBeanDefinitionParser
		//todo 在parseInternal中并不是直接调用自定义的doParse函数，而是进行了一系列的数据准备，
		//todo 包括对beanClass、scope、lazylnit等属性的准备
		...
		if (definition != null && !parserContext.isNested()) {
			try {
				...
				//todo 将 AbstractBeanDefinition 转换为 BeanDefinitionHolder 并注册
				BeanDefinitionHolder holder = new BeanDefinitionHolder(definition, id, aliases);
				//todo 将处理<tx:advice>标签的类BeanDefinition对象，注册到IoC容器中
