...
public abstract class NamespaceHandlerSupport implements NamespaceHandler {
	...
	public BeanDefinition parse(Element element, ParserContext parserContext) {
		//todo NamespaceHandler里面初始化了大量的BeanDefinitionParser来分别处理不同的自定义标签
		//todo 从指定的NamespaceHandler中，匹配到指定的BeanDefinitionParser
		//todo (1)解析过程中首先是寻找元素对应的解析器
		BeanDefinitionParser parser = findParserForElement(element, parserContext);
		//todo 调用指定自定义标签的解析器，完成具体解析工作
		//todo (2)进而调用解析器中的parse方法 AbstractBeanDefinitionParser
	...
	private BeanDefinitionParser findParserForElement(Element element, ParserContext parserContext) {
		//todo 获取自定义标签名，比如<aop:config>标签，此处localName名字是config
		String localName = parserContext.getDelegate().getLocalName(element);
		//todo 根据标签名称找到对应的BeanDefinitionParser
