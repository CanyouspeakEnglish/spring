 ...
 * TODO GenericBeanDefinition只是子类实现,而大部分的通用属性都保存在AbstractBeanDefinition中
 */
...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	/**
	 * todo bean的作用范圈，对应bean属性scope
	 */
	@Nullable
	private String scope = SCOPE_DEFAULT;
	/**
	 * todo 是否是抽象，对应bean属性abstract
	 */
	...
	/**
	 * todo 自动注人模式，对应bean属性autowire
	 */
	private int autowireMode = AUTOWIRE_NO;
	/**
	 * todo 依赖检査，Spring 3.0后弃用这个属性
	 */
	private int dependencyCheck = DEPENDENCY_CHECK_NONE;
	/**
	 * todo 来表示一个bean的实例化依靠另一个bean先实例化，对应bean属性depend-on
	 */
	@Nullable
	private String[] dependsOn;
	/**
	 * todo autowire-candidate属性设置为false,这样容器在盘找自动装配对象时，
	 • TODO 将不考虑该bean.即它不会被考虑作为其他bean自动装配的候选者.但是该bean本身还是可以使用自动 装配来注入其他bean的。
	 • todo 对应 bean 属性 autowire-candidate
	 */
	private boolean autowireCandidate = true;
	/**
	 * todo 自动装配时当出现多个bean候选者时,将作为首选者，对应bean属性primary
	 */
	private boolean primary = false;
	/**
	 * todo 用于记录Qualifier.对应子元素 qualifier
	 */
	...
	/**
	 * todo 允许访问非公开的构造器和方法.程序设置
	 */
	private boolean nonPublicAccessAllowed = true;
	/**
	 * 是否以一种寛松的模式解析构造函數，默认为true,
	 •如果为false,则在如下情况
	 •interface ITest{)
	 •class ITestlmpl implements ITest{};
	 •class Main(
	 •Main(ITest i){}
	 •Main(ITestlmpl i)(}
	 •》
	...
	/**
	 * todo 对应 bean 属性 factory-bean,用法；
	 • <bean id=winstanceFactoryBean1, class=MexaTnple.chapter3.InstanceFactoryBeanM/>
	 • <bean id="currentTime" factory-bean="instianceFactoryBean" factory-method="createTime"/>
	 */
	@Nullable
	private String factoryBeanName;
	/**
	 * todo 对应 bean属性 factory-method
	 */
	@Nullable
	...
	/**
	 * todo 方法重写的持有者，记录lookup-method, replaced-method元素
	 */
	@Nullable
	private MethodOverrides methodOverrides;
	/**
	 * todo 初始化方法，对应bean属性 init-method
	 */
	@Nullable
	private String initMethodName;
	/**
	 * todo 销毁方法.对应bean属性destory-method
	 */
	@Nullable
	...
	/**
	 * todo 是否执行destory-method,程序设置
	 */
	private boolean enforceDestroyMethod = true;
	/**
	 * todo 是否是用户定义的而不是应用程序本身定义的，创建AOP时候为true,程序设置
	 */
	private boolean synthetic = false;
	/**
	 * todo 定义这个bean的应用. APPLICATION：用户.INFRASTRUCTURE：完全内部使用，与用户无关，
	 * todo SUPPORT： 某些复杂配置的一部分
	 • todo 程序设置
	 */
	 ...
	 * @throws BeanDefinitionValidationException in case of validation failure
	 * todo 处理lookup-method, replaced-method
	 * todo 在Spring配 置中存在lookup-method和replace-method两个配置功能，而这两个配置的加载其实就是将配置
	 * todo 统一存放在BeanDefinition中的methodOverrides属性里，这两个功能实现原理其实是在bean实 例化的时候如果检测到存在
	 * todo methodOverrides属性，会动态地为当前bean生成代理并使用对应 的拦截器为bean做増强处理
	 */
	 ...
	 * @throws BeanDefinitionValidationException in case of validation failure
	 * todo 但是，这里要提到的是，对于方法的匹配来讲，如果一个类中存在若干个重栽方法，那么， 在函数调用及增强的时候还需要根据参
	 * todo 数类型进行匹配，来最终确认当前调用的到底是哪个函 数。但是，Spring将一部分冋配工作在这里完成了，
	 * todo 如果当前类中的方法只有一个，那么就设 置重载该方法没有被重载，这样在后续调用的时候便可以直接使用找到的方法，
	 * todo 而不需要进行 方法的参数匹配验证了，而且还可以提前对方法存在性进行验证，正可谓一箭双雕
	 */
	protected void prepareMethodOverride(MethodOverride mo) throws BeanDefinitionValidationException {
		...
		if (count == 0) {
			...
			//todo 标记MethodOverride暂未被覆盖.避免参数类型检査的开销
