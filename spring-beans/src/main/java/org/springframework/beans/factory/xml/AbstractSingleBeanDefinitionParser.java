...
public abstract class AbstractSingleBeanDefinitionParser extends AbstractBeanDefinitionParser {
	 ...
	 * @see #doParse
	 * todo  进行了一系列的数据准备， 包括对beanClass、scope、lazylnit等属性的准备
	 */
	@Override
	protected final AbstractBeanDefinition parseInternal(Element element, ParserContext parserContext) {
		//todo 通过BeanDefinition构建器，创建GenericBeanDefinition对象
		...
		//todo 调用子类的getBeanClass方法，获取BeanDefinition对应的BeanClass
		//todo TxAdviceBeanDefinitionParser子类对应的BeanClass是TransactionInterceptor.class
		Class<?> beanClass = getBeanClass(element);
		//todo 如果BeanClass不为空，则将BeanClass设置到BeanDefinition对象中
		if (beanClass != null) {
			builder.getRawBeanDefinition().setBeanClass(beanClass);
		}
		else {
			//todo 若子类没有重写getBeanClass方法则尝试检査了类是否重写getBeanClassName方法
		...
		//todo 调用子类的doParse方法
		//todo 如果是tx:advice标签，则此处调用的是TxAdviceBeanDefinitionParser类的doParse方法
