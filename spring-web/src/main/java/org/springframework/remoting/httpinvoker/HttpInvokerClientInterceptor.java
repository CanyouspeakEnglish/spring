...
public class HttpInvokerClientInterceptor extends RemoteInvocationBasedAccessor
	...
	/**
	 * todo 该方法所提供的主要功能就是将调用信息封 装在Remoteinvocation中，发送给服务端并等待返回结果
	...
	public Object invoke(MethodInvocation methodInvocation) throws Throwable {
		...
		//todo 将要调用的方法對装为RemoteInvocation  (1 )构建 Remoteinvocation 实例。
		//todo 因为是代理中增强方法的调用，调用的方法及参数信息会在代理中封装至Meihodlnvocation 实例中，
		//todo 并在增强方器中进行传递，也就意味着当程序进入invoke方法时其实是已经包含了调用的接口的相关信息的，那么，
		//todo  首先要做的就是将Methodinvocation中的信息提取并构建 Remoteinvocation 实例
		RemoteInvocation invocation = createRemoteInvocation(methodInvocation);
		RemoteInvocationResult result;

		try {
			//todo 远程执行方法 (2 )远程执行方法
		...
		try {
			//todo 提取结果(3)
			//todo 考虑到序列化的问题，在Spring中约定使用Httplnvokcr方式进行远程方法调用时，结果
			//todo  使用RemotelnvocationResult进行封装，那么在提取结果后还需要从封装的结果中提取对应的结果
