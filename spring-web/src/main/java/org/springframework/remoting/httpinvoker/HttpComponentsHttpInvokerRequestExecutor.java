...
public class HttpComponentsHttpInvokerRequestExecutor extends AbstractHttpInvokerRequestExecutor {
	 ...
	 * @throws java.io.IOException if thrown by I/O methods
	 * todo 设置RequestBody
	 * todo 构建好PostMethod实例后便可以将存储Remoteinvocation实例的序列化对象的输出流设置进去
	 * todo ，当然这里需要注意的是传入的ContcntType类型，一定要传入application/x-java-scrialized-object
	 * todo 以保证服务端解析时会按照序列化对象的解析方式进行解析
	 */
	protected void setRequestBody(
		...
		//todo 将序列化流加入到postMethod中并声明ContentType类型为application/x-java0serialized-object
	...
	protected void validateResponse(HttpInvokerClientConfiguration config, HttpResponse response)
			throws IOException {
		//todo 对于HTTP调用的响应码处理，大于300则是非正常调用的响应码
	 ...
	 * todo 提取响应信息 提取结果的流程主要是从输入流中提取响应的序列化信息
	 */
