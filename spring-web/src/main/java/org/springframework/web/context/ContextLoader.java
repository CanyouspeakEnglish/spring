...
public class ContextLoader {
	...
	static {
		...
		try {
			//todo ContextLoader.properties 加载默认值
			//todo 程序首先会读取ContextLoader类的同目录下的属性文件ContextLoader.properties
			//todo 并根据其中的配置提取将要实现的WebApplicationContext接口的实现类,并根据这个实现类通过反射的方式进行实例的创建
	 ...
	 * todo 初始化WebApplicationContext  WebApplicationContext在Web应用中我们会用到WebApplicationContext
	 * todo WebApplicationContext继承ApplicationContext在ApplicationContext的基础上又追加了一些特定Web的操作及属性
	 * todo 非常类似于我们通过编程方式使用Spring时使用的ClassPathXmlApplicationContext类提供的功能
	 */
	public WebApplicationContext initWebApplicationContext(ServletContext servletContext) {
		if (servletContext.getAttribute(WebApplicationContext.ROOT_WEB_APPLICATION_CONTEXT_ATTRIBUTE) != null) {
			//todo web.xml中存在多次ContextLoader定义 1webApplicationContext存在性的验证
		...
		try {
			...
			if (this.context == null) {
				//todo 初始化Context 2 创建webApplicationContext实例  委托给了createWebApplicationContext
				//todo 3 将实例记录在servletContext中
			...
			//todo 记录servletContext中
			servletContext.setAttribute(WebApplicationContext.ROOT_WEB_APPLICATION_CONTEXT_ATTRIBUTE, this.context);
			//todo 映射当前的类加载器与创建的实例到全局变量currentContextPerThread中
	 ...
	 * @see ConfigurableWebApplicationContext
	 * todo 创建webApplicationContext实例
	 */
